...
/**
 * Compact, resilient settings layer
 * - Reads a local snapshot first (so UI layout persists even if the server 500s)
 * - Syncs to server when available
 * - Never nukes existing local layout on fetch errors
 */
...
export const SettingsProvider = ({ children }) => {
  ...
  // ---------- Tax & Profit helpers ----------
  const calcTax = useCallback((sellPrice) => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        // 1) Local snapshot (ALWAYS apply first)
        try {
          ...
          if (raw) {
            ...
            if (Array.isArray(ls.visible_widgets))
              setVisibleWidgets(
                ls.visible_widgets.filter((k) => ALL_WIDGET_KEYS.includes(k))
              );
            if (Array.isArray(ls.widget_order))
              setWidgetOrder(
                ls.widget_order.filter((k) => ALL_WIDGET_KEYS.includes(k))
              );
        ...
        // 2) Server (best-effort; do not clobber layout on failure)
        ...
        if (p) setPortfolio({ startingCoins: p?.startingBalance ?? 0 });

        // 3) Persist a compact local snapshot (used for resilience)
        const lsNow = {
          visible_widgets:
            (s?.visible_widgets &&
              s.visible_widgets.filter((k) => ALL_WIDGET_KEYS.includes(k))) ||
            visible_widgets,
          widget_order:
            (s?.widget_order &&
              s.widget_order.filter((k) => ALL_WIDGET_KEYS.includes(k))) ||
        ...
        localStorage.setItem("user_settings", JSON.stringify(lsNow));
  ...
  const saveSettings = async (partial) => {
    ...
    // 2) Persist compact snapshot to localStorage (never lose layout)
    ...
    // 3) Server writes (best-effort; do not break UX if they fail)
    try {
      ...
      // Keep local layout even if server fails
      console.warn("Settings save (server) failed:", e?.message || e);
      setError(e);
    }
  };
