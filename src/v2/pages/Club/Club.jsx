...
const fmt = (value) => new Intl.NumberFormat("en-GB").format(Math.round(Number(value) || 0));
  ...
  return (
    <div className="club-page">
      ...
      <section className="club-kpis">
        <Kpi icon={<Coins />} label="Profit today" value={signed(performance.profit_today)} />
        <Kpi icon={<TrendingUp />} label="This week" value={signed(performance.profit_week)} />
      ...
      <section className="club-summary-grid">
        ...
        <div className="club-panel"><PanelTitle eyebrow="AT A GLANCE" title="Trading account" /><div className="account-stats"><Stat label="Open positions" value={performance.open_positions || 0} /><Stat label="Coins invested" value={fmt(invested)} /><Stat label="Total profit" value={signed(performance.total_profit)} /><Stat label="EA tax paid" value={fmt(performance.total_ea_tax)} /><Stat label="Average hold" value={hold(performance.average_hold_hours)} /><Stat label="Closed trades" value={performance.closed_trades || 0} /></div></div>
      </section>
...
function History({ trades }) {
  ...
  return <div className="history-table"><div className="history-head"><span>Player</span><span>Bought</span><span>Sold</span><span>Profit</span><span>Date</span></div>{trades.map((trade) => <div className="history-row" key={trade.trade_id}><div><strong>{trade.player}</strong><small>{trade.version || "Card"} · x{trade.quantity || 1}</small></div><span>{fmt(trade.buy)}</span><span>{fmt(trade.sell)}</span><strong className={Number(trade.profit) >= 0 ? "positive" : "negative"}>{signed(trade.profit)}</strong><span>{date(trade.sold_at)}</span></div>)}</div>;
}

function Performance({ data }) {
  const strategies = data.strategies || [];
  const confidence = data.confidence_accuracy || [];
  return <section className="performance-grid"><div className="club-panel"><PanelTitle eyebrow="WHAT WORKS" title="Strategy performance" />{strategies.length ? strategies.map((row) => <div className="performance-row" key={row.strategy}><div><strong>{label(row.strategy)}</strong><span>{row.wins}/{row.trades} wins</span></div><b>{signed(row.profit)}</b><em>{pct(row.roi)}</em></div>) : <p className="muted">Close a few recommendation-backed trades to compare strategies.</p>}</div><div className="club-panel"><PanelTitle eyebrow="MODEL CHECK" title="Confidence accuracy" />{confidence.length ? confidence.map((row) => <div className="performance-row" key={row.band}><div><strong>{row.band}% confidence</strong><span>{row.wins}/{row.trades} wins</span></div><b>{signed(row.average_profit)} avg</b></div>) : <p className="muted">Confidence tracking begins when recommendation trades are completed.</p>}</div></section>;
}
...
function signed(value) { const n = Number(value || 0); return `${n >= 0 ? "+" : "−"}${fmt(Math.abs(n))}`; }
