...
export default function TraderProfileNew() {
  ...
  const loadTraderProfile = async () => {
    ...
    try {
      const summaryRes = await getTraderRatingSummary(traderId);
      setRatingSummary(summaryRes.data);
    } catch (error) {
    }

    try {
      const myRes = await getMyTraderRating(traderId);
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      await rateTrader(traderId, { rating: ratingValue, review: ratingReview || undefined });
      const summaryRes = await getTraderRatingSummary(traderId);
      setRatingSummary(summaryRes.data);
      const myRes = await getMyTraderRating(traderId);
      setMyRating(myRes.data);
  ...
  const handleDeleteRating = async () => {
    ...
    setRatingLoading(true);
    try {
      await deleteMyTraderRating(traderId);
      const summaryRes = await getTraderRatingSummary(traderId);
      setRatingSummary(summaryRes.data);
