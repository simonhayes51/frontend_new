...
const isBadId = (v) =>
  !v ||
  String(v).trim() === '' ||
  ['undefined', 'null'].includes(String(v).trim().toLowerCase());
...
export default function TraderProfileNew() {
  ...
  // ✅ IMPORTANT: after loading profile, use this internal id for all actions
  const internalTraderId = useMemo(() => {
    if (!trader) return null;
    if (!isBadId(trader.id)) return String(trader.id);
    return null;
  }, [trader]);
  ...
  const socialLinks = useMemo(() => {
    if (!trader) return [];
    return [
      { label: 'Website', value: trader.website_url, icon: Globe },
      { label: 'Twitter', value: trader.twitter_url, icon: Twitter },
      { label: 'YouTube', value: trader.youtube_url, icon: Youtube },
      { label: 'Twitch', value: trader.twitch_url, icon: Twitch },
    ].filter((item) => item.value);
  }, [trader]);

  useEffect(() => {
    if (isBadId(routeTraderId)) return;
    loadTraderProfile();
    ...
    try {
      ...
      if (!isBadId(tid)) {
        ...
        if (myRes.data?.has_rated && myRes.data.rating) {
          ...
          setRatingReview(myRes.data.rating.review || '');
        } else {
          setRatingValue(0);
          setRatingReview('');
        }
  ...
  const handleTierSubscribe = async (tier) => {
    ...
    try {
      if (tier === 'free') {
        if (isBadId(tid)) throw new Error('Missing trader id');
  ...
  const handleSubmitRating = async () => {
    ...
    try {
      await rateTrader(tid, { rating: ratingValue, review: ratingReview || undefined });
  ...
  const handleDeleteRating = async () => {
    ...
    setRatingLoading(true);
    try {
      await deleteMyTraderRating(tid);

      const summaryRes = await getTraderRatingSummary(tid);
      setRatingSummary(summaryRes.data);

      setMyRating({ has_rated: false, rating: null });
      setRatingValue(0);
      setRatingReview('');

      toast.success('Rating removed');
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Failed to remove rating');
    } finally {
      setRatingLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-dark-bg">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    {trader.bio && <p className="text-gray-400 max-w-2xl">{trader.bio}</p>}
                  ...
                  <div>
                    <div className="text-2xl font-bold text-white flex items-center gap-2">
                      <div className="flex items-center gap-1">
                        ...
                        {(ratingSummary?.avg_rating ?? trader.avg_rating ?? 0).toFixed(1)}
                      </div>
                      {ratingSummary?.total_ratings ? (
                        <span className="text-sm text-gray-400">({ratingSummary.total_ratings})</span>
                      ) : null}
                    </div>
                ...
                {/* Specialties */}
                {Array.isArray(trader.specialties) && trader.specialties.length > 0 && (
        ...
        {/* Ratings */}
        <div className="mt-6 bg-dark-card border border-white/10 rounded-3xl shadow-card p-6 md:p-8">
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1">
              <h3 className="text-xl font-bold text-white mb-2">Rate this trader</h3>
              <p className="text-sm text-gray-400 mb-4">
                Share your experience to help others discover high quality traders.
              </p>
                ...
                {Array.from({ length: 5 }).map((_, index) => {
                  ...
                  return (
                    ...
                    >
                      <Star
                        className={`w-7 h-7 ${
                          value <= ratingValue ? 'text-yellow-400' : 'text-slate-600'
                        }`}
                      />
                    </button>
                  );
              ...
              <textarea
                ...
                placeholder="Optional: write a short review about this trader"
                rows={3}
                className="w-full rounded-xl bg-slate-950/60 border border-white/10 px-3 py-2 text-sm text-white placeholder:text-slate-500"
              />
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <button
                  ...
                  className="px-6 py-2 rounded-xl bg-gradient-brand text-white font-semibold hover:shadow-glow-cyan transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                {myRating?.has_rated && (
                  <button
                    ...
                    disabled={ratingLoading}
                    className="px-6 py-2 rounded-xl bg-white/5 hover:bg-red-500/20 text-white hover:text-red-400 font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Remove my rating
                  </button>
                )}
                ...
                <div className="bg-slate-950/50 border border-white/10 rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <div className="text-3xl font-bold text-white">
                        {(ratingSummary.avg_rating ?? 0).toFixed(1)}
                      </div>
                      <div className="text-xs text-gray-400">
                        {ratingSummary.total_ratings || 0} ratings
                      </div>
                    </div>
                    <Star className="w-10 h-10 text-brand-cyan fill-brand-cyan" />
                  </div>
                  ...
                  <div className="space-y-1">
                    {[5, 4, 3, 2, 1].map((star) => {
                      ...
                      const total = ratingSummary.total_ratings || 1;
        ...
        {!isSubscribed && (
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            <TierCard
              ...
              price="$4.99"
              color="basic"
              features={['Access to all posts', 'Comment on content', 'Exclusive trading tips']}
            ...
            {['posts', 'about', 'media'].map((tab) => (
              <button
                ...
                className={`pb-4 px-2 font-semibold capitalize transition-all relative ${
                  activeTab === tab ? 'text-brand-cyan' : 'text-gray-500 hover:text-gray-300'
                }`}
              >
                {tab}
                {activeTab === tab && (
        ...
        <div className="mt-8 pb-12">
          {activeTab === 'posts' && (
            <div className="space-y-6">
              {posts.length === 0 ? (
                ...
                posts.map((post) => <PostCard key={post.id} post={post} trader={trader} />)
              )}
            </div>
          )}
          ...
          {activeTab === 'about' && (
            <div className="bg-dark-card border border-white/10 rounded-2xl p-6">
              ...
              <p className="text-gray-400 mb-6">{trader.bio || 'No bio available'}</p>
                ...
                <>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {socialLinks.map((link) => {
                      const Icon = link.icon || Globe; // ✅ fallback prevents #306
                      return (
                        <a
                          key={link.label}
              ...
              {Array.isArray(trader.specialties) && trader.specialties.length > 0 && (
                <>
                  <h4 className="text-lg font-bold text-white mb-3 mt-6">Specialties</h4>
                  <div className="flex flex-wrap gap-2">
                    {trader.specialties.map((specialty, idx) => (
                      <span key={idx} className="px-4 py-2 bg-white/5 text-gray-300 rounded-xl">
                        {specialty}
                      </span>
                    ))}
                  </div>
...
function PostCard({ post, trader }) {
  const hoverId = trader?.id || trader?.user_id; // ✅ prefer id; fallback just in case

  return (
    <div className="bg-dark-card border border-white/10 rounded-2xl p-6">
      <div className="flex items-center gap-3 mb-4">
        <UserHoverCard userId={hoverId} username={trader.username}>
          <img
            src={trader.avatar_url || '/server-logo.png'}
            alt={trader.username}
