...
const saveLayoutToStorage = (layout) => { try { localStorage.setItem(LAYOUT_STORAGE_KEY, JSON.stringify(layout)); } catch {} };
const loadLayoutFromStorage = () => { try { const s = localStorage.getItem(LAYOUT_STORAGE_KEY); return s ? JSON.parse(s) : null; } catch { return null; } };
const saveWidgetSettingsToStorage = (settings) => { try { localStorage.setItem(WIDGET_SETTINGS_KEY, JSON.stringify(settings)); } catch {} };
const loadWidgetSettingsFromStorage = () => { try { const s = localStorage.getItem(WIDGET_SETTINGS_KEY); return s ? JSON.parse(s) : null; } catch { return null; } };
...
/* ---------- Helpers to restore previously working tiles ---------- */
// Next Promo
function useNextPromo() {
  const [data, setData] = React.useState(null);
  React.useEffect(() => {
    let on = true;
    (async () => {
      try {
        const r = await fetch(`${API_BASE}/api/events/next`, { credentials: "include" });
...
const NextPromoCard = ({ formatDate }) => {
  const data = useNextPromo();
  return (
    <div className={cardBase}>
      <div className={cardTitle}>Next Promo</div>
      {data ? (
        <>
          <div className="text-green-300 text-lg font-bold">{data.name}</div>
          <div className={subText}>
            {data.kind} • {formatDate(new Date(data.start_at))}
            {data.confidence ? ` • ${data.confidence}` : ""}
          </div>
        </>
      ) : (
        <div className="flex-1 flex items-center text-gray-500 text-sm">Loading…</div>
      )}
    </div>
  );
};
...
// Market Summary
function useMarketSummary(tf = "24") {
  const [data, setData] = React.useState(null);
  React.useEffect(() => {
    let on = true;
    (async () => {
      try {
        const r = await fetch(`${API_BASE}/api/market/summary?tf=${tf}`, { credentials: "include" });
        const js = await r.json();
        if (on) setData(js);
      } catch {}
    })();
...
const MarketSummaryCard = () => {
  const ms = useMarketSummary("24");
  return (
    <div className={cardBase}>
      <div className={cardTitle}>Market Summary</div>
      {ms ? (
        <div className="flex-1 flex items-center gap-3 text-sm">
          <span className="text-green-400">📈 {ms.trending}</span>
          <span className="text-red-400">📉 {ms.falling}</span>
          <span className="text-gray-300">≈ {ms.stable}</span>
          <span className="text-gray-500 text-xs">({ms.sample} cards • {ms.tf})</span>
        </div>
      ) : (
        <div className="flex-1 flex items-center text-gray-500 text-sm">Loading…</div>
      )}
      <div className={subText}>Snapshot from momentum pages</div>
    </div>
  );
};

// Trending (6h mini)
function useTrendingMini() {
  const [item, setItem] = React.useState(null);
  React.useEffect(() => {
    let on = true;
    (async () => {
      try {
        const r = await fetch(`${API_BASE}/api/trending?type=fallers&tf=6`, { credentials: "include" });
        const js = await r.json();
        setItem(on ? (js.items?.[0] || null) : null);
      } catch {}
    })();
    return () => { on = false; };
  }, []);
    ...
    <div className={cardBase}>
      ...
      {it ? (
        <div className="flex-1 flex items-center gap-3">
          {it.image ? (
            <img src={it.image} alt="" className="h-10 w-8 object-contain rounded bg-gray-800/60" />
          ) : (
            <div className="h-10 w-8 bg-gray-800 rounded" />
          )}
          <div>
            <div className="text-white font-semibold">
              {it.name}{it.rating ? <span className="text-gray-400"> • {it.rating}</span> : null}
            </div>
            <div className="text-sm" style={{ color: (it.percent ?? 0) > 0 ? ACCENT : "#f87171" }}>
              {(it.percent > 0 ? "+" : "") + Number(it.percent ?? 0).toFixed(2)}%
            </div>
          </div>
        </div>
      ) : (
      ...
      <div className={subText}>
        <Link to="/trending" className="text-blue-400 hover:text-blue-300">Open Trending</Link>
      </div>
    </div>
  );
};
...
const StreakCard = ({ trades }) => {
  ...
  for (let i = 0; i < trades.length; i++) {
    const t = trades[i];
    if ((Number(t.profit) || 0) > 0) streak++;
    else break;
  }
  return (
    <div className={cardBase}>
      <div className={cardTitle}>Current Streak</div>
      <div className="text-green-400"><span className={cardHuge}>{streak}</span></div>
      <div className={subText}>Consecutive wins</div>
    </div>
  );
};

// Performance Score (simple composite)
const PerformanceCard = ({ winRate, avgProfit, totalVolume }) => {
  const score = Math.max(0, Math.min(100,
    0.6 * (Number(winRate) || 0) +
    0.3 * Math.min(40, (Number(avgProfit) || 0) / 1000) +
    0.1 * Math.min(40, (Number(totalVolume) || 0) / 100000)
  ));
  return (
    <div className={cardBase}>
      <div className={cardTitle}>Performance Score</div>
      <div className="text-emerald-400"><span className={cardHuge}>{score.toFixed(0)}</span></div>
      <div className={subText}>Heuristic composite</div>
    </div>
  );
};
...
export default function Dashboard() {
  ...
  const recentTrades = useMemo(() => {
    // sort by timestamp/date desc if present
    const toTime = (t) => {
      const x = t?.timestamp || t?.date;
      try { return new Date(x).getTime(); } catch { return 0; }
    };
    return trades.slice().sort((a, b) => toTime(b) - toTime(a)).slice(0, previewLimit);
  }, [trades, previewLimit]);
    ...
    <PremiumGate feature="smart_insights" featureName="AI Smart Insights">
      <div className={`${cardBase} bg-gradient-to-br from-blue-900/20 to-purple-900/20 border-blue-500/20`}>
        <div className="flex items-center justify-between">
          ...
          <span className={`${chip} bg-blue-500/20 text-blue-300`}><Brain size={10} /> AI</span>
        </div>
    ...
    <PremiumGate feature="advanced_analytics" featureName="Advanced Analytics">
      <div className={`${cardBase} bg-gradient-to-br from-purple-900/20 to-pink-900/20 border-purple-500/20`}>
        <div className="flex items-center justify-between">
          ...
          <span className={`${chip} bg-purple-500/20 text-purple-300`}><BarChart3 size={10} /> PRO</span>
        </div>
    ...
    <PremiumGate feature="market_predictions" featureName="Market Predictions">
      <div className={`${cardBase} bg-gradient-to-br from-green-900/20 to-emerald-900/20 border-green-500/20`}>
        <div className="flex items-center justify-between">
          ...
          <span className={`${chip} bg-green-500/20 text-green-300`}><Activity size={10} /> LIVE</span>
        </div>
  ...
  const renderWidget = (key) => {
    switch (key) {
      ...
      // Restored / core widgets
      case "promo":          return <NextPromoCard formatDate={formatDate} />;
      case "market_summary": return <MarketSummaryCard />;
      case "trending":       return <TrendingMiniCard />;
      case "streak":         return <StreakCard trades={recentTrades} />;
      case "performance":    return <PerformanceCard winRate={winRate} avgProfit={avgProfit} totalVolume={totalVolume} />;
      ...
      case "winrate": return (
        <div className={cardBase}>
          ...
          <div className="text-green-400"><span className={cardHuge}>{winRate.toFixed(1)}%</span></div>
      ...
      case "avg_profit": return (
        <div className={cardBase}>
          ...
          <div className="text-yellow-400"><span className={cardBig}>{formatCurrency(avgProfit)} coins</span></div>
      ...
      case "best_trade": return (
        <div className={cardBase}>
          ...
          <div className="text-green-400"><span className={cardBig}>{formatCurrency(bestTrade)} coins</span></div>
      ...
      case "volume": return (
        <div className={cardBase}>
          ...
          <div className="text-purple-400"><span className={cardBig}>{formatCurrency(totalVolume)} coins</span></div>
      ...
      case "balance": return (
        <div className={cardBase}>
          ...
          <div className="text-gray-300"><span className={cardBig}>{formatCurrency(startingBalance ?? 0)} coins</span></div>
      ...
      case "roi": return (
        <div className={cardBase}>
          ...
          <div className={roi >= 0 ? "text-green-400" : "text-red-400"}><span className={cardHuge}>{roi.toFixed(1)}%</span></div>
        ...
        <div className={cardBase}>
          ...
          <div className="flex-1 flex items-center">
            {alerts && alerts.length > 0 ? (
              <div className="text-sm text-orange-300">{alerts.length} active alert{alerts.length !== 1 ? "s" : ""}</div>
            ) : (
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`text-xs px-3 py-1 rounded-lg border ${editLayout ? "bg-gray-800 border-gray-700" : "bg-gray-900/70 border-gray-800 hover:border-gray-700"} flex items-center gap-1.5`}
              ...
              <div key={idx} className="flex items-center justify-between p-3 bg-gray-800/50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400">{formatDate(new Date(trade.timestamp || trade.date || Date.now()))}</div>
                  </div>
                </div>
